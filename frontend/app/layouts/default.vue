...
<template>
  <div v-if="!initialized" class="center-screen">
    <div class="spinner" />
    <p>Connecting to manager…</p>
  </div>

  <div v-else-if="backendError" class="center-screen">
    <section class="auth-card error-card">
      <div class="brand-mark">λ</div>
      <p class="eyebrow">BACKEND UNAVAILABLE</p>
      <h1>Manager connection failed</h1>
      <p>{{ backendError }}</p>
      <code>{{ apiBase }}</code>
      <button class="primary" @click="manager.initialize">Retry</button>
    </section>
  </div>
  ...
  <main v-else-if="!user" class="auth-shell">
    <section class="auth-card">
      ...
      <p class="muted">{{ bootstrapRequired ? 'Create the first local administrator account.' : 'Sign in to manage local inference.' }}</p>
      <p v-if="authError" class="alert error">{{ authError }}</p>
      <form @submit.prevent="submitAuth">
        ...
        <button class="primary" :disabled="authenticating">{{ authenticating ? 'Working…' : bootstrapRequired ? 'Create admin' : 'Sign in' }}</button>
      </form>
    </section>
  </main>
  ...
  <div v-else class="app-shell">
    <aside class="sidebar">
      <NuxtLink to="/" class="brand"><span>λ</span><strong>llamacpp<br>manager</strong></NuxtLink>
      <nav>
      ...
      <div class="identity">
        ...
        <button class="text-button" @click="manager.logout">Sign out</button>
      </div>
    </aside>
