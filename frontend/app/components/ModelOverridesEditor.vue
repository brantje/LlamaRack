...
<template>
  <div class="space-y-3" data-testid="model-overrides-editor">
    <div v-if="rows.length" class="divide-y divide-[var(--color-divider)] border-y border-[var(--color-divider)]">
      <div
        v-for="row in rows"
        :key="row.id"
        class="grid gap-3 py-3 md:grid-cols-[220px_minmax(0,1fr)_auto] md:items-center"
        data-testid="model-override-row"
      >
        <UInput
          v-model="row.key"
    ...
    <p v-else class="text-xs text-[var(--neutral-700)]">No Model-specific defaults.</p>
    <AppButton v-if="showAddButton" intent="secondary" size="xs" type="button" data-testid="add-model-option" @click="addOption">Add option</AppButton>
  </div>
</template>
