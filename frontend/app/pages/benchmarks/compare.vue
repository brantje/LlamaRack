...
const blockers = computed(() => differences.value.filter(value => value !== 'Runtime configuration'))
const compatible = computed(() => left.value && right.value ? benchmarkComparisonCompatible(left.value, right.value) : false)
...
function delta(a?: number, b?: number) {
  ...
  const change = b - a
  if (a === 0) return `${change >= 0 ? '+' : ''}${change.toFixed(2)} tok/s`
  const percent = change / a * 100
  return `${change >= 0 ? '+' : ''}${change.toFixed(2)} tok/s (${percent >= 0 ? '+' : ''}${percent.toFixed(1)}%)`
}
