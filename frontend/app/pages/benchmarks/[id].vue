...
<template>
  <div class="space-y-6">
    ...
    <template v-else-if="run">
      ...
      <section class="space-y-3">
        <div><h2 class="text-base font-semibold">Measurements</h2><p class="mt-1 text-xs text-muted">Tool-native prompt and generation cases remain separate. Context depth is the KV cache prefill depth used before the measured case.</p></div>
        <Frame class="overflow-hidden"><div class="overflow-x-auto"><table class="w-full min-w-[860px] text-left text-xs"><thead class="border-b border-[var(--color-divider)] text-muted"><tr><th class="px-4 py-3 font-medium">Case</th><th class="px-4 py-3 font-medium">Prompt tokens</th><th class="px-4 py-3 font-medium">Generation tokens</th><th class="px-4 py-3 font-medium">Context depth</th><th class="px-4 py-3 font-medium">Repetitions</th><th class="px-4 py-3 font-medium">Average</th><th class="px-4 py-3 font-medium">Stddev</th></tr></thead><tbody class="divide-y divide-[var(--color-divider)]"><tr v-for="result in run.results || []" :key="`${result.case_index}:${result.case_id}`"><td class="px-4 py-3 font-mono">{{ result.case_id }}</td><td class="px-4 py-3 font-mono tabular-nums">{{ result.prompt_tokens }}</td><td class="px-4 py-3 font-mono tabular-nums">{{ result.generation_tokens }}</td><td class="px-4 py-3 font-mono tabular-nums">{{ benchmarkResultDepth(result) }}</td><td class="px-4 py-3 font-mono tabular-nums">{{ result.repetitions }}</td><td class="px-4 py-3 font-mono tabular-nums">{{ formatRate(result.average_tokens_per_second) }}</td><td class="px-4 py-3 font-mono tabular-nums">{{ formatRate(result.stddev_tokens_per_second) }}</td></tr><tr v-if="!run.results?.length"><td colspan="7" class="px-4 py-8 text-center text-muted">No successful measurements were stored for this run.</td></tr></tbody></table></div></Frame>
      </section>
