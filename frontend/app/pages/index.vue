...
<template>
  <div>
    ...
    <section class="panel"><div class="panel-header"><div><p class="eyebrow">FLEET</p><h2>Model activity</h2></div><NuxtLink v-if="canOperate" to="/models" class="button primary small">Manage models</NuxtLink></div><div v-if="!models.length" class="empty-state"><strong>No models configured</strong><p>Add a local GGUF model to start serving requests.</p></div><div v-else class="table-wrap"><table><thead><tr><th>Model</th><th>Status</th><th>Loading</th><th>Priority</th><th>GGUF</th></tr></thead><tbody><tr v-for="model in models" :key="model.id"><td><strong>{{ model.model_id }}</strong><small>{{ model.name }}</small></td><td><span class="status" :data-state="manager.modelState(model)">{{ manager.modelState(model) }}</span></td><td>{{ model.always_on ? 'Always on' : model.autoload_enabled ? 'Autoload' : 'Manual' }}</td><td>{{ model.priority }}</td><td class="mono">{{ model.gguf_path }}</td></tr></tbody></table></div></section>
  </div>
</template>
