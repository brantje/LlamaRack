...
type Companion = { kind: string; path: string; flag: string; size?: number }
type VRAMSegment = { label: string; bytes: number; percent: number; token: string }

type RuntimeWithTime = ReturnType<ReturnType<typeof useManager>['runtimeForInstance']> & {
  started_at?: string
  ready_at?: string
}
...
const liveTelemetry = computed(() => manager.observabilityLive.value?.telemetry || [])
...
const contextPercent = computed(() => contextUsed.value !== undefined && model.value?.context_length
  ? Math.min(100, Math.max(0, contextUsed.value / model.value.context_length * 100))
  : undefined)
const runtimeStartedAt = computed(() => runtime.value?.started_at ? new Date(runtime.value.started_at) : undefined)
const runtimeUptime = computed(() => runtimeStartedAt.value && Number.isFinite(runtimeStartedAt.value.getTime()) && ['STARTING', 'LOADING', 'READY', 'STOPPING'].includes(runtime.value?.state || '')
  ? Math.max(0, Date.now() - runtimeStartedAt.value.getTime())
  : undefined)

const relevantGPUIds = computed(() => {
  const ids = telemetry.value?.gpu_devices?.length
    ? telemetry.value.gpu_devices
    : telemetry.value?.gpus?.map(gpu => gpu.device_id) || []
  return new Set(ids)
})
const relevantGPUs = computed(() => {
  const gpus = hardware.value?.gpus || []
  if (!relevantGPUIds.value.size) return gpus
  return gpus.filter(gpu => relevantGPUIds.value.has(gpu.id))
})
const relevantVRAMTotal = computed(() => relevantGPUs.value.reduce((total, gpu) => total + Math.max(0, gpu.total_bytes), 0))
const currentVRAM = computed(() => telemetry.value?.vram_used_bytes)
const currentVRAMPercent = computed(() => currentVRAM.value !== undefined && relevantVRAMTotal.value > 0 ? Math.min(100, currentVRAM.value / relevantVRAMTotal.value * 100) : undefined)
...
const contextChart = computed(() => gapFilled(contextSeries.value, value => model.value?.context_length ? Math.min(100, Math.max(0, value / model.value.context_length * 100)) : Number.NaN))

const companions = computed<Companion[]>(() => {
  const current = model.value as any
  if (!current) return []
  if (Array.isArray(current.helpers)) {
    return current.helpers
      .filter((helper: any) => helper?.path)
      .map((helper: any) => ({ kind: helper.kind === 'mtp' ? 'MTP draft model' : 'Vision projector', path: helper.path, flag: helper.flag || (helper.kind === 'mtp' ? '--model-draft' : '--mmproj'), size: helper.size ?? helper.total_bytes }))
  }
  const options = current.options || {}
  const out: Companion[] = []
  const mmproj = options.mmproj || options['--mmproj']
  const draft = options['model-draft'] || options['--model-draft'] || options.draft_model
  if (mmproj) out.push({ kind: 'Vision projector', path: String(mmproj), flag: '--mmproj' })
  if (draft) out.push({ kind: 'MTP draft model', path: String(draft), flag: '--model-draft' })
  return out
})
...
function vramSegments(gpu: NonNullable<typeof hardware.value>['gpus'][number]): VRAMSegment[] {
  const attributed = liveTelemetry.value
    .map(sample => {
      const value = sample.gpus?.find(item => item.device_id === gpu.id)?.vram_used_bytes
      return value !== undefined ? { id: sample.instance_id, bytes: Math.max(0, value) } : undefined
    })
    .filter((item): item is { id: string; bytes: number } => Boolean(item?.bytes))
    .sort((a, b) => a.id === instanceID.value ? -1 : b.id === instanceID.value ? 1 : a.id.localeCompare(b.id))
  const attributedBytes = attributed.reduce((total, item) => total + item.bytes, 0)
  const unattributed = Math.max(0, gpu.used_bytes - attributedBytes)
  const free = Math.max(0, gpu.free_bytes)
  const total = Math.max(1, gpu.total_bytes)
  const segments: VRAMSegment[] = attributed.map((item, index) => ({
    label: item.id === instanceID.value ? `${item.id} (this Instance)` : item.id,
    bytes: item.bytes,
    percent: Math.min(100, item.bytes / total * 100),
    token: item.id === instanceID.value ? 'var(--color-accent)' : instanceColorTokens[index % instanceColorTokens.length]!
  }))
  if (unattributed > 0) segments.push({ label: 'Unattributed process memory', bytes: unattributed, percent: Math.min(100, unattributed / total * 100), token: 'var(--neutral-600)' })
  segments.push({ label: 'Free', bytes: free, percent: Math.min(100, free / total * 100), token: 'var(--neutral-400)' })
  return segments
}
...
function historyPath(metric: string) {
  const query = new URLSearchParams({
    metric,
    window_seconds: String(selectedWindow.value),
    bucket_seconds: String(bucketSeconds.value),
  ...
  <div class="space-y-8">
    <div class="flex flex-wrap items-start justify-between gap-5">
      ...
      <div v-if="instance" class="flex flex-wrap items-center justify-end gap-2">
        ...
        <AppButton
          v-if="['UNLOADED', 'FAILED'].includes(runtime?.state || 'UNLOADED')"
          intent="primary"
          :loading="pending === 'start'"
          @click="runtimeAction('start')"
        >Launch</AppButton>
    ...
    <template v-else-if="instance">
      <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4" data-testid="instance-detail-summary">
        ...
        <Frame class="p-4">
          ...
          <div class="mt-3 space-y-1 text-[11px] text-[var(--neutral-800)]"><p>Utilization <span class="float-right font-mono tabular-nums text-[var(--color-text)]">{{ formatPercent(currentVRAMPercent) }}</span></p><p>Placement <span class="float-right font-mono text-[var(--color-text)]">{{ telemetry?.gpu_devices?.length ? telemetry.gpu_devices.join(', ') : '—' }}</span></p></div>
        </Frame>
      </section>
      ...
      <section data-testid="instance-detail-vram-allocation" class="space-y-3">
        ...
        <div v-if="relevantGPUs.length" class="grid gap-4 lg:grid-cols-2">
          <Frame v-for="gpu in relevantGPUs" :key="gpu.id" class="p-4">
      ...
      <Frame class="p-4" data-testid="instance-detail-runtime">
        ...
        <dl class="mt-4 grid gap-0 border-t border-l border-[var(--color-divider)] sm:grid-cols-2 lg:grid-cols-4">
          <div v-for="item in [
            ['PID', runtime?.pid || '—'], ['Port', runtime?.port || '—'], ['Placed on', telemetry?.gpu_devices?.length ? telemetry.gpu_devices.join(', ') : '—'],
            [gpuUsageLabel, formatPercent(telemetry?.gpu_utilization_pct)], ['VRAM', formatBytes(telemetry?.vram_used_bytes)], ['CPU', formatPercent(telemetry?.cpu_percent)],
