...
async function action(instance: Instance, operation: 'start' | 'stop' | 'restart' | 'kill' | 'duplicate') {
  if (operation === 'start' && instance.eviction_enabled) {
    if (!confirm('Launching this Instance may stop other idle Instances if resource-pressure eviction is required. Continue?')) return
  }
  if (operation === 'kill' && !confirm('Kill this Instance immediately? Active requests may fail.')) return
...
async function remove(instance: Instance) {
  if (!confirm(`Delete Instance “${instance.name}”? The registered Model and GGUF file are kept.`)) return
  ...
  <div class="space-y-5">
    <div class="flex items-start justify-between gap-6">
      ...
      <div class="flex flex-wrap justify-end gap-2">
        ...
        <UButton to="/instances/new">New Instance</UButton>
      </div>
    </div>
    ...
    <UEmpty v-if="!instances.length" title="No Instances configured" description="Create an Instance for a registered Model. Stopped Instances remain here and can be launched later.">
      <template #actions><UButton to="/instances/new" size="sm">New Instance</UButton></template>
    </UEmpty>
        ...
        <div class="space-y-4">
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              ...
              <p class="mt-1 break-all font-mono text-xs text-muted">model={{ instance.id }}</p>
              <p class="mt-1 text-sm text-muted">{{ modelName(instance.model_id) }}</p>
            </div>
            <UBadge :color="manager.instanceState(instance) === 'READY' ? 'success' : manager.instanceState(instance) === 'FAILED' ? 'error' : 'neutral'" variant="subtle">
              {{ manager.instanceState(instance) }}
            </UBadge>
          </div>

          <div class="grid grid-cols-2 gap-2 text-xs text-muted">
            <span>Priority: {{ instance.priority }}</span>
            <span>GPU: {{ instance.gpu_mode }}</span>
            <span>{{ instance.always_on ? 'Always On' : 'Not Always On' }}</span>
            <span>{{ instance.autoload_enabled ? 'Autoload' : 'Manual load' }}</span>
            <span class="col-span-2">{{ instance.eviction_enabled ? 'Resource-pressure eviction allowed' : 'Protected from resource-pressure eviction' }}</span>
          </div>
    ...
    <UModal v-model:open="logsOpen" :title="logTitle">
      <template #body>
        <pre class="max-h-[65vh] overflow-auto whitespace-pre-wrap rounded-md bg-elevated p-4 font-mono text-xs">{{ logLines.length ? logLines.join('\n') : 'No logs captured.' }}</pre>
      </template>
    </UModal>
  </div>
</template>
