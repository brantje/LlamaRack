...
type HFArtifact = { id: string; name: string; quantization?: string; total_bytes: number; shard_count: number; expected_shards: number; complete: boolean; files: HFFile[] }
...
<template>
  <div class="space-y-5">
    ...
    <template v-else-if="selected">
      ...
      <UCard>
        <div class="mb-4"><p class="mb-1 text-xs font-extrabold tracking-[0.18em] text-dimmed">GGUF ARTIFACTS</p><h2 class="text-xl font-bold">Available quantizations</h2></div>
        <div v-if="!selected.artifacts.length" class="flex items-center gap-3 py-8 text-muted"><UIcon name="i-lucide-file-x" class="size-5" /><span>No GGUF files found</span></div>
        <div v-else class="divide-y divide-default">
          <div v-for="artifact in selected.artifacts" :key="artifact.id" class="grid gap-4 py-4 lg:grid-cols-[minmax(0,1fr)_140px_140px_auto] lg:items-center">
            <div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><span class="truncate font-semibold">{{ artifact.name }}</span><UBadge v-if="artifact.quantization" color="primary" variant="subtle">{{ artifact.quantization }}</UBadge><UBadge v-if="artifact.shard_count > 1" color="neutral" variant="soft">{{ artifact.shard_count }} shards</UBadge><UBadge v-if="!artifact.complete" color="error" variant="subtle">Incomplete split</UBadge></div><p class="mt-1 truncate font-mono text-xs text-dimmed">{{ artifact.files.map(file => file.path).join(', ') }}</p></div>
            <div class="text-sm text-muted">{{ formatBytes(artifact.total_bytes) }}</div><div class="text-sm text-muted">{{ artifact.complete ? 'Ready to download' : `${artifact.shard_count}/${artifact.expected_shards} shards` }}</div><UButton :disabled="!artifact.complete" :loading="downloading === artifact.id" @click="download(artifact)">Download</UButton>
          </div>
        </div>
      </UCard>
    </template>
