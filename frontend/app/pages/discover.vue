...
async function search() {
  ...
  try {
    ...
    error.value = value?.data?.error || value?.message || 'Unable to search Hugging Face'
  } finally {
    loading.value = false
  }
}
...
async function openModel(id: string) {
  ...
  try {
    selected.value = await manager.request<HFDetail>(`/api/v1/huggingface/model?repo=${encodeURIComponent(id)}`)
  } catch (value: any) {
...
async function download(artifact: HFArtifact) {
  ...
  downloadNotice.value = ''
  try {
    await manager.request('/api/v1/downloads', {
      method: 'POST',
      body: { repo_id: selected.value.id, artifact_id: artifact.id }
    })
    downloadNotice.value = `${artifact.name} was added to Downloads.`
  } catch (value: any) {
...
<template>
  <div class="space-y-5">
    ...
    <UCard>
      <UForm :state="{}" class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_220px_220px_auto]" @submit="search">
        <UFormField label="Search" name="search">
          <UInput v-model="query" class="w-full" placeholder="Qwen, Llama, Gemma…" icon="i-lucide-search" />
        </UFormField>
        ...
        <div class="flex items-end">
          <UButton class="w-full justify-center" type="submit" :loading="loading">Search</UButton>
        </div>
      </UForm>
    </UCard>
    ...
    <div v-if="loading" class="grid gap-3 xl:grid-cols-2">
      <USkeleton v-for="n in 6" :key="n" class="h-36 w-full rounded-xl" />
    </div>
      ...
      <UCard v-for="item in results" :key="item.id" class="cursor-pointer transition hover:ring-1 hover:ring-primary" @click="openModel(item.id)">
        <div class="flex items-start justify-between gap-4">
          <div class="min-w-0">
            ...
            <p class="mt-1 text-sm text-muted">{{ item.downloads.toLocaleString() }} downloads · {{ item.likes.toLocaleString() }} likes</p>
          </div>
          <div class="flex gap-2">
            <UBadge v-if="item.private" color="warning" variant="subtle">Private</UBadge>
            <UBadge v-if="item.gated" color="warning" variant="subtle">Gated</UBadge>
            <UBadge color="primary" variant="subtle">GGUF</UBadge>
          </div>
        </div>
        <div class="mt-4 flex flex-wrap gap-1.5">
          <UBadge v-for="tag in (item.tags || []).slice(0, 5)" :key="tag" color="neutral" variant="soft">{{ tag }}</UBadge>
        </div>
      </UCard>
    </div>
    ...
    <template v-else-if="selected">
      ...
      <UCard>
        <div class="flex flex-wrap items-start justify-between gap-4">
          ...
          <div class="flex gap-2">
            <UBadge v-if="selected.private" color="warning">Private</UBadge>
            <UBadge v-if="selected.gated" color="warning">Gated</UBadge>
          </div>
        </div>
      </UCard>
      ...
      <UCard>
        ...
        <div v-if="!selected.artifacts.length" class="py-8">
          <UEmpty icon="i-lucide-file-x" title="No GGUF files found" />
        </div>
        <div v-else class="divide-y divide-default">
          <div v-for="artifact in selected.artifacts" :key="artifact.id" class="grid gap-4 py-4 lg:grid-cols-[minmax(0,1fr)_140px_140px_auto] lg:items-center">
            <div class="min-w-0">
              ...
              <p class="mt-1 truncate font-mono text-xs text-dimmed">{{ artifact.files.map(file => file.path).join(', ') }}</p>
            </div>
            <div class="text-sm text-muted">{{ formatBytes(artifact.total_bytes) }}</div>
