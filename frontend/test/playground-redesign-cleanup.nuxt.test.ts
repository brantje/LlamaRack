...
describe('Playground redesign cleanup', () => {
  it('pins the chat composer to the bottom of a viewport-locked thread', () => {
    ...
    expect(source).toContain('grid min-h-0 flex-1 gap-4 overflow-y-auto xl:grid-cols-[minmax(0,1fr)_20rem] xl:items-stretch xl:overflow-hidden')
    expect(source).toContain('flex min-h-[calc(100dvh-12rem)] min-w-0 flex-col overflow-hidden p-0 xl:h-full xl:min-h-0')
