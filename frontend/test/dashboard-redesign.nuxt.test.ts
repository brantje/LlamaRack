...
function resetManager() {
  ...
  manager.instances.value = [{
    ...
    gpu_mode: 'auto', gpu_devices: [], tensor_split: '', request_log_mode: 'metadata'
  }]
  ...
  manager.observabilityLive.value = {
    ...
    hardware: {
      ...
      collected_at: '2026-08-29T18:00:00Z',
      processes: [],
      gpus: [{ id: 'CUDA0', backend: 'cuda', index: 0, name: 'GPU', total_bytes: 16 * gib, used_bytes: 12 * gib, free_bytes: 4 * gib, utilization_pct: 50 }]
    },
...
function requestRecord() {
  return {
    ...
    api_key: { id: 'k1', name: 'Primary', prefix: 'pk_1' },
    streaming: true,
    status_code: 200,
    result: 'success',
    duration_ms: 1500,
...
describe('Dashboard redesign', () => {
  it('renders the new flat dashboard hierarchy and retention-aware selector', async () => {
    mocks.request.mockImplementation(async (path: string) => {
      if (path === '/api/v1/settings/general') return {
        ...
        observability_retention_days: { value: 1, source: 'database', editable: true }
      }
    ...
    expect(wrapper.get('[data-testid="dashboard-observability-kpis"]').text()).toContain('Tokens · 15 min')
    expect(wrapper.get('[data-testid="dashboard-observability-kpis"]').text()).toContain('3.5')
    expect(wrapper.get('[data-testid="dashboard-observability-kpis"]').text()).toContain('90')
    expect(wrapper.get('[data-testid="dashboard-observability-kpis"]').text()).toContain('2 active')
    expect(wrapper.get('[data-testid="dashboard-observability-kpis"]').text()).toContain('1 streaming')
  ...
  it('refreshes summary and request history immediately when the range changes', async () => {
    mocks.request.mockImplementation(async (path: string) => {
      if (path === '/api/v1/settings/general') return {
        idle_unload_seconds: { value: 0, source: 'default', editable: true },
    ...
    const select = wrapper.findAllComponents({ name: 'Select' }).find(component => component.attributes('data-testid') === 'dashboard-range')
      ?? wrapper.findAllComponents({ name: 'USelect' }).find(component => component.attributes('data-testid') === 'dashboard-range')
    expect(select).toBeTruthy()
    select!.vm.$emit('update:modelValue', 3600)
    await flushPromises()
