...
describe('Profile machine typography', () => {
  it('uses mono tabular treatment for account timestamps', () => {
    expect(accountSource.match(/font-mono text-\[length:var\(--font-size-h6\)\] tabular-nums/g)).toHaveLength(2)
  })

  it('uses mono tabular treatment for authentication timestamps', () => {
    expect(authenticationSource).toContain('font-mono text-[length:var(--font-size-table-header)] tabular-nums text-[var(--neutral-700)]">Linked {{ profileDateTime(identity.created_at) }}')
  })

  it('uses mono tabular treatment for session metadata', () => {
    expect(sessionsSource).toContain('break-words font-mono text-[length:var(--font-size-table-header)] leading-5 tabular-nums text-[var(--neutral-700)]')
