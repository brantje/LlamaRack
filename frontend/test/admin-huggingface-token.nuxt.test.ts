...
describe('Hugging Face administration', () => {
  ...
  it('loads and saves the Hugging Face max download size in human units as bytes', async () => {
    ...
    expect(wrapper.text()).toContain('Persisted as 1,099,511,627,776 bytes')
    ...
    await flushPromises()
    expect(wrapper.text()).toContain('Persisted as 2,199,023,255,552 bytes')
  ...
  it('splits loaded byte ceilings into GiB and MiB and ignores unchanged or invalid saves', async () => {
    ...
    await flushPromises()
    expect(gib.text()).toContain('Persisted as 2,147,483,648 bytes')
    ...
    await flushPromises()
    expect(gib.text()).toContain('Persisted as 0 B')
    await button(gib, 'Save download limit').trigger('click')
    await flushPromises()
    expect(mocks.request).not.toHaveBeenCalledWith('/api/v1/huggingface/settings', expect.objectContaining({ method: 'PUT' }))
    ...
    await flushPromises()
    expect(mib.text()).toContain('Persisted as 536,870,912 bytes')
    unitSelect(mib).vm.$emit('update:modelValue', 'GiB')
    await flushPromises()
    expect(mib.text()).toContain('Persisted as 549,755,813,888 bytes')
    mib.unmount()
    ...
    await flushPromises()
    expect(fallback.text()).toContain('Persisted as 1,099,511,627,776 bytes')
