...
describe('AppConfirmationModal', () => {
  it('uses default labels and tone when optional values are omitted', async () => {
  ...
  it('cancels an outstanding request when a new confirmation replaces it', async () => {
    ...
    const first = (wrapper.vm as any).request({
      title: 'First action', description: 'First description', confirmLabel: 'First confirm', cancelLabel: 'First cancel', confirmTone: 'destructive'
    })
    await flushPromises()
    const second = (wrapper.vm as any).request({
      title: 'Second action', description: 'Second description', confirmLabel: 'Second confirm', cancelLabel: 'Second cancel', confirmTone: 'destructive'
    })
    await flushPromises()
