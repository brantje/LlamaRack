...
describe('redesign feedback regressions', () => {
  ...
  it('keeps reviewed mobile action hierarchy and overflow affordances', () => {
    ...
    const modelForm = readFileSync(resolve(process.cwd(), 'app/components/ModelForm.vue'), 'utf8')
    ...
    expect(api).toContain('class="min-w-[960px]"')
    expect(api).not.toContain('Revoke')
    expect(instanceForm).toContain('data-testid="instance-submit-requirements"')
    expect(instanceForm).toContain(":color=\"form.priority === priority ? 'primary' : 'neutral'\"")
    expect(instanceForm).toContain(":variant=\"form.priority === priority ? 'solid' : 'ghost'\"")
    ...
    expect(modelForm).toContain("'model-submit-requirements'")
