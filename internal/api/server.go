...
func (s *Server) serveAuthenticated(w http.ResponseWriter, r *http.Request, path string, user auth.User) {
	...
	case path == "/api/v1/providers/huggingface/search" && r.Method == http.MethodGet:
		items,err:=s.hf.Search(r.Context(),r.URL.Query().Get("q"),30);if err!=nil{writeErr(w,502,err);return};writeJSON(w,200,items)
	case strings.HasPrefix(path,"/api/v1/providers/huggingface/models/") && strings.HasSuffix(path,"/files") && r.Method == http.MethodGet:
		repo:=strings.TrimSuffix(strings.TrimPrefix(path,"/api/v1/providers/huggingface/models/"),"/files");items,err:=s.hf.Files(r.Context(),repo);if err!=nil{writeErr(w,502,err);return};writeJSON(w,200,items)
	case path == "/api/v1/downloads" && r.Method == http.MethodGet:
		writeJSON(w,200,s.downloads.List())
	case path == "/api/v1/downloads/huggingface" && r.Method == http.MethodPost:
		if !requireOperate(w,user){return};var in struct{Repo string `json:"repo"`;File string `json:"file"`;Revision string `json:"revision"`};if !decode(w,r,&in){return};job,err:=s.downloads.StartHuggingFace(in.Repo,in.File,in.Revision);if err!=nil{writeErr(w,400,err);return};writeJSON(w,202,job)
	case path == "/api/v1/downloads/url" && r.Method == http.MethodPost:
		if !requireOperate(w,user){return};var in struct{URL string `json:"url"`};if !decode(w,r,&in){return};job,err:=s.downloads.StartURL(in.URL);if err!=nil{writeErr(w,400,err);return};writeJSON(w,202,job)
	case strings.HasPrefix(path,"/api/v1/downloads/") && strings.HasSuffix(path,"/cancel") && r.Method == http.MethodPost:
		if !requireOperate(w,user){return};id:=strings.TrimSuffix(strings.TrimPrefix(path,"/api/v1/downloads/"),"/cancel");if err:=s.downloads.Cancel(id);err!=nil{writeErr(w,404,err);return};w.WriteHeader(204)
...
func (s *Server) modelSubresource(w http.ResponseWriter,r *http.Request,path string,user auth.User) {
	rest:=strings.TrimPrefix(path,"/api/v1/models/");parts:=strings.Split(rest,"/")
	if len(parts)<1||parts[0]==""{writeJSON(w,404,map[string]string{"error":"not found"});return}
	id:=parts[0]
	...
	switch parts[1] {
	case "start":
		if r.Method!=http.MethodPost{w.WriteHeader(405);return};if !requireOperate(w,user){return};_,err:=s.lifecycle.StartModel(r.Context(),id);if err!=nil{writeErr(w,503,err);return};runtime,_:=s.lifecycle.RuntimeForModel(r.Context(),id);writeJSON(w,202,runtime)
	case "stop":
		if r.Method!=http.MethodPost{w.WriteHeader(405);return};if !requireOperate(w,user){return};if err:=s.lifecycle.StopModel(r.Context(),id);err!=nil{writeErr(w,500,err);return};w.WriteHeader(204)
	case "runtime":
		if r.Method!=http.MethodGet{w.WriteHeader(405);return};items,err:=s.lifecycle.RuntimeForModel(r.Context(),id);if err!=nil{writeErr(w,500,err);return};writeJSON(w,200,items)
	case "options":
