...
func TestDynamicCORSAllowsConfiguredAndSameOriginRequests(t *testing.T) {
	...
	for _, origin := range []string{"https://manager.example.com", "http://manager.local"} {
		...
		if w.Header().Get("Access-Control-Allow-Credentials") != "" || w.Header().Get("Vary") != "Origin" {
			t.Fatalf("origin %q unexpected CORS response headers: %v", origin, w.Header())
		}
		...
		if !strings.Contains(w.Header().Get("Access-Control-Allow-Headers"), "Authorization") || strings.Contains(w.Header().Get("Access-Control-Allow-Headers"), "X-CSRF-Token") {
			t.Fatalf("origin %q unexpected allowed headers: %q", origin, w.Header().Get("Access-Control-Allow-Headers"))
		}
		if !strings.Contains(w.Header().Get("Access-Control-Expose-Headers"), "X-LiteLLM-Trace-ID") {
			t.Fatalf("origin %q trace header is not exposed: %q", origin, w.Header().Get("Access-Control-Expose-Headers"))
		}
	}
	if called != 2 {
