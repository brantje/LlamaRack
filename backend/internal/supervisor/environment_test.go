...
func TestSanitizeLlamaArgEnvironment(t *testing.T) {
	...
	if err := os.Setenv("LLAMARACK_TEST_KEEP", "yes"); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
		_ = os.Unsetenv("LLAMA_ARG_CTX_SIZE")
		_ = os.Unsetenv("LLAMA_ARG_FLASH_ATTN")
		_ = os.Unsetenv("LLAMARACK_TEST_KEEP")
	})
	...
	if got := os.Getenv("LLAMARACK_TEST_KEEP"); got != "yes" {
...
func TestWorkerEnvironOverridesVisibleDevices(t *testing.T) {
	...
	t.Setenv("LLAMARACK_TEST_KEEP", "yes")
