...
func TestAnalyzeFullPartialHybridCPUAndTotalFit(t *testing.T) {
	...
	if !strings.Contains(full.Quantization.Summary, "Balanced") { t.Fatalf("quant=%+v", full.Quantization) }
	...
	if !partial.CurrentFit || partial.Offload.Mode != "partial" || !partial.Offload.KVOnGPU || partial.Offload.GPULayers <= 0 || !partial.TotalHardwareFit { t.Fatalf("partial=%+v", partial) }
	...
	if !hybrid.CurrentFit || hybrid.Offload.Mode != "hybrid" || hybrid.Offload.KVOnGPU || hybrid.Offload.GPULayers != 32 { t.Fatalf("hybrid=%+v", hybrid) }

	cpu := Analyze(model, metaPath, hardware.Snapshot{RAMAvailableBytes: 8 * gib, RAMTotalBytes: 16 * gib}, 0, errors.New("gpu probe failed"))
	if !cpu.CurrentFit || !cpu.CPUFit || cpu.Offload.Mode != "cpu" || cpu.ContextLength != defaultContext || !cpu.ContextAssumed || cpu.HardwareWarning == "" { t.Fatalf("cpu=%+v", cpu) }
}
...
func TestRecommendationDecisionBranches(t *testing.T) {
	...
	if !fit || multi.Mode != "multi_gpu" || len(multi.Devices) != 2 || multi.TensorSplit == "" { t.Fatalf("multi=%+v fit=%v", multi, fit) }
	...
	if !fit || partial.Mode != "partial" || partial.GPULayers != 0 { t.Fatalf("partial=%+v", partial) }
	...
	if !fit || tiny.Mode != "cpu" { t.Fatalf("tiny=%+v", tiny) }
	fit, noRAM := recommendOffload(hardware.Snapshot{GPUs: []hardware.GPU{{ID: "CUDA0", FreeBytes: 128 * mib, TotalBytes: 8 * gib}}}, memory, m)
	if fit || noRAM.Mode != "cpu" { t.Fatalf("noRAM=%+v", noRAM) }
	if fit, empty := recommendOffload(hardware.Snapshot{}, memory, m); fit || empty.Mode != "" { t.Fatalf("empty=%+v", empty) }
}

func TestHelpersAndQuantizationBranches(t *testing.T) {
	if got, assumed := chooseContext(16384); got != 16384 || assumed { t.Fatalf("context=%d %v", got, assumed) }
	if got, assumed := chooseContext(0); got != defaultContext || !assumed { t.Fatalf("default=%d %v", got, assumed) }
	if got := estimateMemory(-1, 4096, Metadata{}); got.WeightsBytes != 0 || got.RuntimeOverheadBytes != 256*mib { t.Fatalf("memory=%+v", got) }
	if estimateKV(0, Metadata{}) != 0 { t.Fatal("zero kv") }
	if estimateKV(4096, Metadata{BlockCount: 2, Embedding: 16, HeadCount: 4}) <= 0 { t.Fatal("fallback kv heads") }
	if estimateKV(4096, Metadata{BlockCount: 2, Embedding: 2, HeadCount: 4}) != 0 { t.Fatal("invalid head dimension") }
	if confidence(Metadata{Architecture: "llama"}, errors.New("partial")) != "medium" || confidence(Metadata{}, errors.New("bad")) != "low" { t.Fatal("confidence") }
	if recommendedLayers(0, .5) != 0 || recommendedLayers(32, 0) != 0 || recommendedLayers(32, .001) != 1 || recommendedLayers(32, 2) != 32 { t.Fatal("layer bounds") }
	if fitsRAM(defaultRAMReserve, 0) || !fitsRAM(2*defaultRAMReserve, defaultRAMReserve) { t.Fatal("RAM boundary") }
...
func TestAnalyzeAndReadMetadataGracefulFallback(t *testing.T) {
	...
	if r.Confidence != "low" || r.MetadataWarning == "" || !r.ContextAssumed { t.Fatalf("fallback=%+v", r) }
	if _, err := ReadMetadata(filepath.Join(t.TempDir(), "missing.gguf")); err == nil { t.Fatal("missing should fail") }
	...
	if err != nil || m.Architecture != "gemma3" || m.ContextLength != 131072 || m.BlockCount != 24 { t.Fatalf("metadata=%+v err=%v", m, err) }
}
