...
func TestReconcileSkipsDisabledAndAlreadyReady(t *testing.T) {
	...
	if sup.Status(instances[0].ID).State != supervisor.Ready { t.Fatal("always-on worker not ready") }
	s.ReconcileAlwaysOn(ctx)
	if sup.Status(instances[0].ID).State != supervisor.Ready { t.Fatal("ready worker changed state") }
}
