...
func TestOptionArgs(t *testing.T) {
	got := optionArgs(map[string]string{
		"z": "3", "--alpha": "1", "flag": "TRUE", "off": "false", "blank": " ",
	})
...
func TestEnsureReadyPoliciesAndMissingModel(t *testing.T) {
	ctx := context.Background()
...
func TestNoEnabledInstanceAndAlwaysOnReconciliation(t *testing.T) {
	...
	if logs := s2.Logs(instances[0].ID); logs == nil {
		// nil is allowed if the worker has not emitted output; invoke method for coverage.
	}
	_ = ms
}
