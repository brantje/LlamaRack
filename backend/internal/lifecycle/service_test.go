...
func TestLifecycleHelperProcess(t *testing.T) {
	if os.Getenv("GO_WANT_LIFECYCLE_HELPER") != "1" { return }
	args := os.Args
	start := 0
...
func TestOptionArgs(t *testing.T) {
	...
	if strings.Join(got, "|") != strings.Join(want, "|") { t.Fatalf("optionArgs=%v want=%v", got, want) }
}
...
func TestEnsureReadyAutoloadStartStopRuntimeAndSingleFlight(t *testing.T) {
	...
	for i := 0; i < callers; i++ {
		wg.Add(1)
		go func() { defer wg.Done(); ep, err := s.EnsureReady(ctx, m.PublicID); if err != nil { errs <- err; return }; results <- ep }()
	}
	wg.Wait(); close(results); close(errs)
	for err := range errs { t.Fatal(err) }
	var endpoint string
	for ep := range results { if endpoint == "" { endpoint = ep }; if ep != endpoint { t.Fatalf("different endpoints: %q vs %q", ep, endpoint) } }
	if endpoint == "" { t.Fatal("missing endpoint") }
	instances, err := s.Runtime(ctx, m.ID)
	if err != nil || len(instances) != 1 || instances[0].State != supervisor.Ready { t.Fatalf("runtime=%+v err=%v", instances, err) }
	second, err := s.StartModel(ctx, m.ID)
	if err != nil || second != endpoint { t.Fatalf("second start=%q err=%v", second, err) }
	modelInstances, _ := s.models.Instances(ctx, m.ID)
	if got := sup.Status(modelInstances[0].ID); got.State != supervisor.Ready { t.Fatalf("supervisor status=%+v", got) }
	if err := s.StopModel(ctx, m.ID); err != nil { t.Fatal(err) }
	instances, _ = s.Runtime(ctx, m.ID)
	if instances[0].State != supervisor.Unloaded { t.Fatalf("stopped runtime=%+v", instances) }
}

func TestEnsureReadyPoliciesAndMissingInstance(t *testing.T) {
	ctx := context.Background()
	s, _, m, _, exec := setupLifecycle(t, false, false)
	if _, err := s.EnsureReady(ctx, "missing"); err == nil { t.Fatal("expected missing instance error") }
	if _, err := s.EnsureReady(ctx, m.PublicID); err == nil || !strings.Contains(err.Error(), "autoload disabled") { t.Fatalf("expected autoload disabled error, got %v", err) }
	exec("UPDATE instances SET enabled=0 WHERE model_id=?", m.ID)
	if _, err := s.EnsureReady(ctx, m.PublicID); err == nil || !strings.Contains(err.Error(), "disabled") { t.Fatalf("expected disabled error, got %v", err) }
}
...
func TestNoEnabledInstanceAndAlwaysOnReconciliation(t *testing.T) {
	...
	exec("UPDATE instances SET enabled=0 WHERE model_id=?", m.ID)
	if _, err := s.StartModel(ctx, m.ID); err == nil || !strings.Contains(err.Error(), "no enabled instance") { t.Fatalf("expected no instance error, got %v", err) }
	...
	for time.Now().Before(deadline) && sup2.Status(instances[0].ID).State != supervisor.Ready { time.Sleep(20 * time.Millisecond) }
	if got := sup2.Status(instances[0].ID); got.State != supervisor.Ready { t.Fatalf("always-on status=%+v", got) }
...
func TestRunReconcilerStopsWithContext(t *testing.T) {
	for _, interval := range []time.Duration{0, 15 * time.Second} {
		t.Run(interval.String(), func(t *testing.T) {
			...
			go func() { s.RunReconciler(ctx, interval); close(done) }()
			cancel()
