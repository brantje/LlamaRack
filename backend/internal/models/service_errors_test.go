...
func TestInstanceGPUParsingAndClosedDatabaseErrors(t *testing.T) {
	...
	if _, err := s.db.ExecContext(ctx, "UPDATE instances SET gpu_mode='manual',gpu_devices='0,1',tensor_split='1,1' WHERE model_id=?", m.ID); err != nil { t.Fatal(err) }
	...
	if _, err := s.GetByPublicID(ctx, "gpu-model"); err == nil { t.Fatal("expected get by public id DB error") }
