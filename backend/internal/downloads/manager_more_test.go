...
VALUES('badrepo','huggingface','bad','rev','a','x.gguf','',?,1,0,0,'',unixepoch(),unixepoch())`, StateQueued)
	...
	_, err = downloadTestDB(t, manager).ExecContext(context.Background(), `INSERT INTO download_files(job_id,path,size,oid,state,downloaded_bytes,etag,ordinal,local_path) VALUES('badrepo','x.gguf',1,'',?,0,'',0,'')`, StateQueued)
	if err != nil {
		t.Fatal(err)
	}
