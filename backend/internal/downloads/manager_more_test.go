...
func TestHeadFailureAndShortBodyFail(t *testing.T) {
	t.Run("head status", func(t *testing.T) {
		...
		failed := waitJob(t, manager, job.ID, StateFailed)
		if !strings.Contains(failed.Error, "metadata request returned HTTP 403") { t.Fatalf("error = %q", failed.Error) }
	})
	...
	t.Run("short body", func(t *testing.T) {
		manager, _, _ := newTestManager(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if r.Method != http.MethodHead { _, _ = io.WriteString(w, "abc") }
		}))
		detail, selected := artifact("acme/demo", "rev", "short", huggingface.File{Path: "demo.gguf", Size: 6})
		job, err := manager.CreateHuggingFace(context.Background(), detail, selected)
		if err != nil { t.Fatal(err) }
		failed := waitJob(t, manager, job.ID, StateFailed)
		if !strings.Contains(failed.Error, "expected 6 bytes, received 3") { t.Fatalf("error = %q", failed.Error) }
	})
}
...
func TestOversizedOrUnverifiablePartialRestarts(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := os.WriteFile(finalPath+".lcm-restart.part", []byte(tc.partial), 0o644); err != nil { t.Fatal(err) }
			if err := manager.ResumePending(context.Background()); err != nil { t.Fatal(err) }
			waitJob(t, manager, "restart", StateCompleted)
			if seenRange != "" { t.Fatalf("unexpected range = %q", seenRange) }
		})
	}
}
...
func TestCompletedFileValidation(t *testing.T) {
	...
	if manager.completedFileValid(job, file) { t.Fatal("missing file reported valid") }
	path, _ := manager.localPath(job, file.Path)
	_ = os.MkdirAll(filepath.Dir(path), 0o755)
	_ = os.WriteFile(path, []byte("xx"), 0o644)
	if manager.completedFileValid(job, file) { t.Fatal("wrong-size file reported valid") }
	_ = os.WriteFile(path, []byte("abc"), 0o644)
	if !manager.completedFileValid(job, file) { t.Fatal("complete file reported invalid") }
}
