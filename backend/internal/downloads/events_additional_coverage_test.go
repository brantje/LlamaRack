...
func TestDetailedListReturnsFileQueryError(t *testing.T) {
	...
	if _, err := downloadTestDB(t, manager).ExecContext(context.Background(), `DROP TABLE download_files`); err != nil {
		t.Fatal(err)
	}
...
func TestRemoveReturnsDeleteDatabaseError(t *testing.T) {
	...
	if _, err := downloadTestDB(t, manager).ExecContext(context.Background(), `CREATE TRIGGER fail_download_delete BEFORE DELETE ON download_jobs BEGIN SELECT RAISE(ABORT, 'blocked'); END`); err != nil {
		t.Fatal(err)
	}
...
func TestRemoveHandlesIgnoredDelete(t *testing.T) {
	...
	if _, err := downloadTestDB(t, manager).ExecContext(context.Background(), `CREATE TRIGGER ignore_download_delete BEFORE DELETE ON download_jobs BEGIN SELECT RAISE(IGNORE); END`); err != nil {
		t.Fatal(err)
	}
