...
func TestHardwarePersistenceTimeseriesLifecycleAndMetrics(t *testing.T) {
	...
	if len(latest.Hardware.GPUs) != 2 || len(latest.Telemetry) != 1 || latest.Telemetry[0].InstanceID != "coder" {
		t.Fatalf("latest=%+v", latest)
	}
	latest.Hardware.GPUs[0].Name = "changed"
	if s.LatestHardware().Hardware.GPUs[0].Name != "GPU A" {
		t.Fatal("latest snapshot must be copied")
	}
	...
	for _, tc := range []struct {
		...
		if err != nil || len(items) != tc.want {
			t.Fatalf("%s/%s/%s items=%+v err=%v", tc.metric, tc.device, tc.instance, items, err)
		}
	}
	if _, err := s.HardwareTimeseries(ctx, "bogus", 0, 0, "", ""); err == nil {
		t.Fatal("expected unsupported hardware metric")
	}

	if err := s.RecordRequest(ctx, RequestRecord{StartedAt: now.UnixMilli(), FinishedAt: now.UnixMilli(), InstanceID: "coder", Endpoint: "/v1/chat/completions", StatusCode: 200, Autoloaded: true, LoadDurationMS: 125}); err != nil {
		t.Fatal(err)
	}
	if err := s.RecordRequest(ctx, RequestRecord{StartedAt: now.UnixMilli(), FinishedAt: now.UnixMilli(), InstanceID: "coder", Endpoint: "/v1/chat/completions", StatusCode: 503, Autoloaded: true, LoadDurationMS: 75}); err != nil {
		t.Fatal(err)
	}
	lifecycle, err := s.LifecycleSummary(ctx, 0)
	if err != nil || lifecycle.Autoloads != 2 || lifecycle.FailedStarts != 1 || lifecycle.LoadMS != 200 {
	...
	if w.Code != 200 || !strings.Contains(w.Body.String(), `"hardware"`) || !strings.Contains(w.Body.String(), `"autoloads":2`) {
	...
	if w.Code != 200 || !strings.Contains(w.Body.String(), "CUDA0") {
		t.Fatalf("hardware series=%d %s", w.Code, w.Body.String())
	}
	...
	body := w.Body.String()
	for _, want := range []string{"llamarack_ram_used_bytes", `device_id="CUDA0"`, "llamarack_instance_cpu_percent", "llamarack_autoload_total"} {
...
func TestHardwarePruneAndRetentionStop(t *testing.T) {
	...
	if err := s.RecordHardware(ctx, old, nil); err != nil {
		t.Fatal(err)
	}
	if err := s.PruneHardware(ctx, 30); err != nil {
		t.Fatal(err)
	}
	items, err := s.HardwareTimeseries(ctx, "ram_used_bytes", time.Now().Add(-60*24*time.Hour).UnixMilli(), 60, "", "")
	if err != nil || len(items) != 0 {
		t.Fatalf("old items=%+v err=%v", items, err)
	}
	...
	case <-time.After(time.Second):
		t.Fatal("hardware retention did not stop")
	}
}
