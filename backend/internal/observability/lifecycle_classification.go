...
func classifyLifecycle(previous, next supervisor.Runtime, state lifecycle.OperationalState) []lifecycleTransition {
	if next.InstanceID == "" || previous.State == next.State {
		return nil
	}
	...
	if next.State == supervisor.Ready {
		...
		if !next.StartedAt.IsZero() && !next.ReadyAt.IsZero() && next.ReadyAt.After(next.StartedAt) {
			duration = next.ReadyAt.Sub(next.StartedAt)
		}
