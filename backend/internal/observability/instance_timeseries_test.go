...
func TestRequestTimeseriesScopesAndBucketsByInstance(t *testing.T) {
	...
	if err != nil || len(requests) != 2 || requests[0].Value != 2 || requests[1].Value != 1 {
		t.Fatalf("requests=%+v err=%v", requests, err)
	}
	prompt, err := s.RequestTimeseries(ctx, "prompt_tokens", now.Add(-5*time.Minute).UnixMilli(), 60, "alpha")
	if err != nil || len(prompt) != 2 || prompt[0].Value != 15 || prompt[1].Value != 2 {
	...
	if err != nil || generated[0].Value != 35 {
	...
	if err != nil || len(p50) != 2 || p50[0].Value != 100 {
		t.Fatalf("p50=%+v err=%v", p50, err)
	}
	p95, err := s.RequestTimeseries(ctx, "latency_p95", now.Add(-5*time.Minute).UnixMilli(), 60, "alpha")
	if err != nil || len(p95) != 2 || p95[0].Value != 300 {
		t.Fatalf("p95=%+v err=%v", p95, err)
	}
	if _, err := s.RequestTimeseries(ctx, "unsupported", 0, 0, "alpha"); err == nil {
...
func TestContextHistoryPersistsOnlyDerivedGauge(t *testing.T) {
	...
	if err != nil || len(points) != 1 || points[0].Value != 4096 {
		t.Fatalf("context=%+v err=%v", points, err)
	}
	other, err := s.RequestTimeseries(ctx, "instance_context_tokens_max", at.Add(-time.Minute).UnixMilli(), 60, "beta")
	if err != nil || len(other) != 0 {
		t.Fatalf("negative context should not persist: %+v err=%v", other, err)
	}
}
...
func TestContextHistoryBucketsUseMaxWatermark(t *testing.T) {
	...
	points, err := s.RequestTimeseries(ctx, "instance_context_tokens_max", at.Add(-time.Minute).UnixMilli(), 60, "alpha")
	if err != nil || len(points) != 1 || points[0].Value != 8000 {
		t.Fatalf("context watermark=%+v err=%v", points, err)
	}
}
...
func TestManagementTimeseriesPassesInstanceFilter(t *testing.T) {
	...
	for _, id := range []string{"alpha", "beta"} {
		if err := s.RecordRequest(ctx, RequestRecord{StartedAt: now.UnixMilli(), FinishedAt: now.UnixMilli(), InstanceID: id, Endpoint: "/v1/responses", StatusCode: 200, DurationMS: 10, PromptTokens: 1, GeneratedTokens: 2, TotalTokens: 3}); err != nil {
			t.Fatal(err)
		}
	}
	...
	if len(response.Items) != 1 || response.Items[0].Value != 1 {
