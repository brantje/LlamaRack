...
`
	...
	if opts[3].Key != "invalid" || opts[3].Kind != "boolean" {
		t.Fatalf("flag option=%+v", opts[3])
	}
}
...
`
	...
	if p.Path != path || p.Version != "llama.cpp test-version" || p.Fingerprint == "" || len(p.Options) != 3 {
		t.Fatalf("profile=%+v", p)
	}
	if p.Options[0].Kind != "integer" || !p.Options[1].ManagerOwned || p.Options[2].Kind != "boolean" {
