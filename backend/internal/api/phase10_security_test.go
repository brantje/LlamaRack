...
func newPhase10SecurityFixture(t *testing.T) *phase10SecurityFixture {
	...
	return &phase10SecurityFixture{
		handler:   ManagementSecurity(authService, network, mux),
		auth:      authService,
...
func TestPhase10BootstrapLoginCSRFAndLogout(t *testing.T) {
	...
	w = phase10Request(t, f.handler, http.MethodPost, "/api/v1/auth/bootstrap", map[string]string{
	...
	w = phase10Request(t, f.handler, http.MethodPost, "/api/v1/auth/login", map[string]string{
		"username": "admin", "password": "correct-horse-battery",
	...
	if w.Code != http.StatusOK || !strings.Contains(w.Body.String(), `"username":"admin"`) {
		t.Fatalf("login status=%d body=%s", w.Code, w.Body.String())
	}
	session := responseCookie(t, w, sessionCookie)
	csrf := responseCookie(t, w, csrfCookie)
	if !session.HttpOnly || csrf.HttpOnly || session.MaxAge < 1 || csrf.MaxAge < 1 {
		t.Fatalf("unexpected cookie policy session=%+v csrf=%+v", session, csrf)
	}

	w = phase10Request(t, f.handler, http.MethodGet, "/api/v1/protected", nil, []*http.Cookie{session}, nil)
	...
	w = phase10Request(t, f.handler, http.MethodPost, "/api/v1/protected", nil, []*http.Cookie{session}, nil)
	...
	w = phase10Request(t, f.handler, http.MethodPost, "/api/v1/protected", nil, []*http.Cookie{session, csrf}, map[string]string{"X-CSRF-Token": csrf.Value})
	if w.Code != http.StatusOK {
		t.Fatalf("csrf protected status=%d body=%s", w.Code, w.Body.String())
	}

	w = phase10Request(t, f.handler, http.MethodPost, "/api/v1/auth/logout", nil, []*http.Cookie{session, csrf}, map[string]string{"X-CSRF-Token": csrf.Value})
	if w.Code != http.StatusNoContent {
		t.Fatalf("logout status=%d body=%s", w.Code, w.Body.String())
	}
	clearedSession := responseCookie(t, w, sessionCookie)
	clearedCSRF := responseCookie(t, w, csrfCookie)
	if clearedSession.MaxAge != -1 || clearedCSRF.MaxAge != -1 {
		t.Fatalf("cookies not cleared session=%+v csrf=%+v", clearedSession, clearedCSRF)
	}
