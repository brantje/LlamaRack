...
func TestPhase10OIDCAdminProviderSettingsAndIdentityRoutes(t *testing.T) {
	...
	w = phase10Request(t, f.secured, http.MethodDelete, "/api/v1/me/identities/"+identity.ID, nil, nil, f.headers())
	...
	w = phase10Request(t, f.secured, http.MethodPost, "/api/v1/admin/auth/identities", map[string]any{"user_id": 1, "provider_id": provider.ID, "issuer": provider.Issuer, "subject": "admin-subject"}, nil, f.headers())
	if w.Code != http.StatusCreated {
