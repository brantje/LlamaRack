...
func TestPhase10ManagementClosedDatabaseErrorPaths(t *testing.T) {
	root := t.TempDir()
	db, err := database.Open(t.Context(), filepath.Join(root, "closed-phase10.db"))
	if err != nil {
		t.Fatal(err)
	}
	...
	request := func(method, path string, body any, invoke func(*httptest.ResponseRecorder, *http.Request)) *httptest.ResponseRecorder {
		t.Helper()
		base := phase10Request(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
