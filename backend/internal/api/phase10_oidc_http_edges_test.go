...
func TestPhase10OIDCAdditionalHTTPBranches(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		w := phase10Request(t, f.raw, tc.method, tc.path, tc.body, nil, nil)
		if w.Code != tc.status {
...
func TestPhase10OIDCClosedDatabaseErrors(t *testing.T) {
	...
	h := NewPhase10OIDCHandler(oidcManager, authService, managerSettings, network)
	if err := db.Close(); err != nil {
		t.Fatal(err)
	}

	for _, path := range []string{"/api/v1/auth/providers", "/api/v1/admin/auth/settings"} {
		w := phase10Request(t, h, http.MethodGet, path, nil, nil, nil)
