...
func TestPhase55InstanceAndModelRouteErrors(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{http.MethodPut, "/api/v1/instances/disabled-instance", map[string]any{"model_id": model.ID, "name": "!!!", "confirm_model_id_change": true}, http.StatusBadRequest},
...
func TestPhase55ModelCreateValidationBranches(t *testing.T) {
	f := newAPIFixture(t, nil)
	cookie := bootstrapAndLogin(t, f)
