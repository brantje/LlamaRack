...
type phase10OIDCHandler struct {
...
func NewPhase10OIDCHandler(oidc *auth.OIDCManager, a *auth.Service, managerSettings *settings.Service, network *managersecurity.Network) http.Handler {
	return &phase10OIDCHandler{oidc: oidc, auth: a, settings: managerSettings, network: network}
}

func (h *phase10OIDCHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
...
func (h *phase10OIDCHandler) publicProviders(w http.ResponseWriter, r *http.Request) {
...
func (h *phase10OIDCHandler) writeAuthSettings(w http.ResponseWriter, r *http.Request) {
...
func (h *phase10OIDCHandler) authSettings(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet { h.writeAuthSettings(w, r); return }
	...
	if in.LocalLoginEnabled != nil && !*in.LocalLoginEnabled {
		...
		if err != nil { writeErr(w, http.StatusInternalServerError, err); return }
		if !usable { writeJSON(w, http.StatusConflict, map[string]string{"error": auth.ErrAuthLockoutRisk.Error()}); return }
	}
	updates := map[string]any{}
	if in.LocalLoginEnabled != nil { updates[settings.LocalLoginEnabled] = *in.LocalLoginEnabled }
...
func (h *phase10OIDCHandler) oidcFlow(w http.ResponseWriter, r *http.Request, rest string) {
	...
	case "start":
		...
		if parseErr != nil || parsed.Query().Get("state") == "" { writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "unable to initialize OIDC state"}); return }
...
func (h *phase10OIDCHandler) exchange(w http.ResponseWriter, r *http.Request) {
	var in struct { Code string `json:"code"` }
	if !decode(w, r, &in) { return }
	result, err := h.oidc.Exchange(in.Code)
	if err != nil { writeJSON(w, http.StatusUnauthorized, map[string]string{"error": "invalid or expired OIDC exchange code"}); return }
	writeJSON(w, http.StatusOK, result)
}

func (h *phase10OIDCHandler) wsTicket(w http.ResponseWriter, r *http.Request) {
...
func (h *phase10OIDCHandler) providers(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		providers, err := h.oidc.ListProviders(r.Context()); if err != nil { writeErr(w, http.StatusInternalServerError, err); return }; writeJSON(w, http.StatusOK, providers)
	case http.MethodPost:
		var in auth.OIDCProviderInput; if !decode(w, r, &in) { return }
		provider, err := h.oidc.CreateProvider(r.Context(), in); if err != nil { writeErr(w, http.StatusBadRequest, err); return }
		slog.Info("security event", "event", "auth.oidc_provider_created", "provider_id", provider.ID); writeJSON(w, http.StatusCreated, provider)
	default: w.WriteHeader(http.StatusMethodNotAllowed)
	}
}

func (h *phase10OIDCHandler) provider(w http.ResponseWriter, r *http.Request, rest string) {
	parts := strings.Split(rest, "/")
	if len(parts) < 1 || parts[0] == "" || len(parts) > 2 { writeJSON(w, http.StatusNotFound, map[string]string{"error": "not found"}); return }
	id := parts[0]
	if len(parts) == 2 {
...
func (h *phase10OIDCHandler) identities(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		items, err := h.oidc.ListIdentities(r.Context(), 0); if err != nil { writeErr(w, http.StatusInternalServerError, err); return }; writeJSON(w, http.StatusOK, items)
	case http.MethodPost:
		var in struct { UserID int64 `json:"user_id"`; ProviderID string `json:"provider_id"`; Issuer string `json:"issuer"`; Subject string `json:"subject"` }
		if !decode(w, r, &in) { return }
		identity, err := h.oidc.LinkIdentity(r.Context(), in.UserID, in.ProviderID, in.Issuer, in.Subject); if err != nil { writeErr(w, http.StatusBadRequest, err); return }
		slog.Info("security event", "event", "auth.external_identity_linked", "user_id", identity.UserID, "provider_id", identity.ProviderID); writeJSON(w, http.StatusCreated, identity)
	default: w.WriteHeader(http.StatusMethodNotAllowed)
	}
}

func (h *phase10OIDCHandler) identity(w http.ResponseWriter, r *http.Request, id string) {
	if r.Method != http.MethodDelete || id == "" || strings.Contains(id, "/") { w.WriteHeader(http.StatusMethodNotAllowed); return }
	if err := h.oidc.UnlinkIdentity(r.Context(), id); err != nil { writeOIDCNotFound(w, err, "external identity not found"); return }
	slog.Info("security event", "event", "auth.external_identity_unlinked", "identity_id", id); w.WriteHeader(http.StatusNoContent)
}

func writeOIDCNotFound(w http.ResponseWriter, err error, message string) {
	if errors.Is(err, sql.ErrNoRows) { writeJSON(w, http.StatusNotFound, map[string]string{"error": message}); return }
