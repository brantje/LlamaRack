...
func phase7Profile() (llamacpp.Profile, error) {
...
func TestPhase7HardwareHandler(t *testing.T) {
	f := newAPIFixture(t, nil)
	cookie := bootstrapAndLogin(t, f)
	handler := NewPhase7HardwareHandler(f.auth, staticHardware{snapshot: hardware.Snapshot{
	...
	failed := NewPhase7HardwareHandler(f.auth, staticHardware{err: errors.New("probe failed")})
...
func TestLlamaConfigHandlerGlobalAndEffectiveValues(t *testing.T) {
	...
	handler := NewLlamaConfigHandler(f.auth, store, phase7Profile)
