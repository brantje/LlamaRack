...
func TestHuggingFaceRemoveDownloadRoutes(t *testing.T) {
	fixture := newHuggingFaceFixture(t)
	if got := huggingFaceRequest(t, fixture, http.MethodDelete, "/api/v1/downloads/missing", nil, true).Code; got != http.StatusNotFound {
	...
	if got := huggingFaceRequest(t, fixture, http.MethodPatch, "/api/v1/downloads/"+job.ID, nil, true).Code; got != http.StatusMethodNotAllowed {
