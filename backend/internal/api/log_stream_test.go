...
func TestAPILogHelperProcess(t *testing.T) {
	if os.Getenv("GO_WANT_API_LOG_HELPER") != "1" { return }
	args := os.Args; start := 0
	...
	mux := http.NewServeMux(); mux.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusOK) })
...
func TestWorkerLogStreamStreamsLiveWorkerOutput(t *testing.T) {
	...
	t.Cleanup(func() { ctx, cancel := context.WithTimeout(context.Background(), time.Second); defer cancel(); sup.Shutdown(ctx) })
	...
	if got := recorder.Header().Get("Content-Type"); got != "text/event-stream" { t.Fatalf("content-type=%q", got) }
	body := recorder.Body.String()
	if !strings.Contains(body, ": connected") || !strings.Contains(body, "fake api worker online") { t.Fatalf("stream body=%q", body) }
}
