...
func TestPhase10ManagementRouteEdges(t *testing.T) {
	f := newPhase10Fixture(t)
...
func TestPhase10APIKeyRouteEdges(t *testing.T) {
...
func TestPhase10AuthHandlerRejectedInputsAndSecureCookies(t *testing.T) {
	...
	w = phase10Request(t, authHandler, http.MethodPost, "/api/v1/auth/bootstrap", map[string]string{"username": "other", "password": "correct-horse-battery"}, nil, nil)
	if w.Code != http.StatusBadRequest {
		t.Fatalf("duplicate bootstrap status=%d body=%s", w.Code, w.Body.String())
	}
	w = phase10Request(t, authHandler, http.MethodPost, "/api/v1/auth/login", []string{"invalid"}, nil, nil)
	if w.Code != http.StatusBadRequest {
		t.Fatalf("invalid login body status=%d body=%s", w.Code, w.Body.String())
	}
	w = phase10Request(t, authHandler, http.MethodGet, "/api/v1/auth/logout", nil, nil, nil)
	if w.Code != http.StatusNotFound {
		t.Fatalf("wrong logout method status=%d body=%s", w.Code, w.Body.String())
	}
	w = phase10Request(t, authHandler, http.MethodPost, "/api/v1/auth/logout", nil, nil, nil)
