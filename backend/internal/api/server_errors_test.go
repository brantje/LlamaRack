...
func TestPersistenceFailuresBecomeHTTPErrorResponses(t *testing.T) {
	...
	admin := auth.User{ID:1, Username:"admin", Role:"admin", Enabled:true}
	...
	for _, tc := range []struct{ method, path string; body any; want int }{
		...
		w := doRequest(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request){ s.authenticated(w,r,tc.path,admin) }), tc.method, tc.path, tc.body, nil)
		if w.Code != tc.want { t.Fatalf("%s %s=%d body=%s", tc.method, tc.path, w.Code, w.Body.String()) }
	}
	...
	for _, tc := range []struct{ method, path string; want int }{
		...
		w := doRequest(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request){ s.modelRoute(w,r,tc.path,admin) }), tc.method, tc.path, nil, nil)
		if w.Code != tc.want { t.Fatalf("%s %s=%d body=%s", tc.method, tc.path, w.Code, w.Body.String()) }
	}

	w := doRequest(t, s, http.MethodGet, "/api/v1/auth/bootstrap", nil, nil)
	if w.Code != 500 { t.Fatalf("bootstrap state closed DB=%d", w.Code) }
	...
	if w.Code != 400 { t.Fatalf("bootstrap closed DB=%d", w.Code) }
	w = doRequest(t, s, http.MethodPost, "/api/v1/auth/login", map[string]string{"username":"admin","password":"correct-horse-battery"}, nil)
	if w.Code != 401 { t.Fatalf("login closed DB=%d", w.Code) }
}

func TestReadonlyModelMutationsAreForbidden(t *testing.T) {
	f := newAPIFixture(t, nil)
	cookie := bootstrapAndLogin(t, f, "readonly")
	for _, tc := range []struct{ method, path string }{
		{http.MethodDelete, "/api/v1/models/id"},
		{http.MethodPost, "/api/v1/models/id/start"},
		{http.MethodPost, "/api/v1/models/id/stop"},
	} {
		w := doRequest(t, f.server, tc.method, tc.path, nil, cookie)
		if w.Code != 403 { t.Fatalf("%s %s=%d body=%s", tc.method, tc.path, w.Code, w.Body.String()) }
	}
}
