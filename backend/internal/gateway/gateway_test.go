...
func TestGatewayHelperProcess(t *testing.T) {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Authorization") != "" {
			w.WriteHeader(500)
			_, _ = w.Write([]byte(`{"error":"authorization leaked"}`))
			return
		}
...
func newGatewayFixture(t *testing.T, autoload bool) *gatewayFixture {
	...
	model, err := m.Create(ctx, models.CreateModelInput{PublicID: "gateway-model", ArtifactID: artifact.ID, Autoload: &autoload})
	if err != nil {
		t.Fatal(err)
	}
	...
	return &gatewayFixture{gateway: New(a, m, l), auth: a, models: m, model: model, secret: secret, sup: sup}
}

func gatewayRequest(t *testing.T, g http.Handler, method, path, secret string, body string) *httptest.ResponseRecorder {
	t.Helper()
