...
func TestUsageParsingJSONSSETimings(t *testing.T) {
	...
	if usage.prompt != 7 || usage.generated != 8 || usage.total != 15 || usage.generationTPS == nil || *usage.generationTPS != 9.5 || usage.promptTPS == nil || *usage.promptTPS != 11.5 { t.Fatalf("timings=%+v", usage) }
...
func TestPersistHelper(t *testing.T) {
	...
	if err != nil || correlated.ID != items[0].ID { t.Fatalf("correlated=%+v err=%v", correlated, err) }
...
func TestNumberAndUsageHelpers(t *testing.T) {
	...
	if value.promptTPS == nil || value.generationTPS == nil || *value.promptTPS != 500 || *value.generationTPS != 500 { t.Fatalf("derived timings=%+v", value) }
}
