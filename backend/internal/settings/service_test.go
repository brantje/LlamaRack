...
func testSettings(t *testing.T) *Service {
	...
	return New(db, Defaults{
		...
		IdleUnloadTimeout: 300 * time.Second, AlwaysOnReconcile: 15 * time.Second,
...
func TestGeneralDefaultsAndDatabaseOverrides(t *testing.T) {
	...
	if _, err := s.Set(ctx, LoginProtectionEnabled, false); err != nil { t.Fatal(err) }
	if got, err := s.Bool(ctx, LoginProtectionEnabled); err != nil || got { t.Fatalf("bool=%v err=%v", got, err) }
	if _, err := s.Set(ctx, TrustedProxies, "10.0.0.0/8"); err != nil { t.Fatal(err) }
...
func TestEnvironmentOverridesDatabaseAndValidation(t *testing.T) {
	ctx := context.Background()
	s := testSettings(t)
	if _, err := s.Set(ctx, LoginFailureThreshold, 7); err != nil { t.Fatal(err) }
	...
	if _, err := s.Set(ctx, LoginFailureThreshold, 8); err == nil { t.Fatal("environment-controlled setting should reject writes") }
	t.Setenv("LCM_LOGIN_FAILURE_THRESHOLD", "invalid")
	if _, err := s.Resolve(ctx, LoginFailureThreshold); err == nil { t.Fatal("invalid environment value should fail") }
}
...
func TestSettingValidationAndTypeErrors(t *testing.T) {
	...
	if _, err := s.Set(ctx, LoginProtectionEnabled, "not-bool"); err == nil { t.Fatal("expected bool validation error") }
	if _, err := s.Set(ctx, "missing", "x"); err == nil { t.Fatal("expected unknown setting error") }
	if _, err := s.Resolve(ctx, "missing"); err == nil { t.Fatal("expected unknown resolve error") }
	if _, err := s.Int(ctx, TrustedProxies); err == nil { t.Fatal("expected integer type error") }
	if _, err := s.String(ctx, SessionLifetimeSeconds); err == nil { t.Fatal("expected string type error") }
	if _, err := s.Bool(ctx, TrustedProxies); err == nil { t.Fatal("expected bool type error") }
}
