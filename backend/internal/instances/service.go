...
func (s *Service) Create(ctx context.Context, in CreateInput) (Instance, error) {
	...
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `INSERT INTO instances(id,model_id,name,enabled,autoload_enabled,always_on,priority,eviction_enabled,idle_unload_seconds,gpu_mode,gpu_devices,tensor_split) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
		i.ID, i.ModelID, i.Name, boolInt(i.Enabled), boolInt(i.Autoload), boolInt(i.AlwaysOn), i.Priority, boolInt(i.EvictionEnabled), i.IdleUnloadSeconds, i.GPUMode, joinDevices(i.GPUDevices), nullString(i.TensorSplit)); err != nil {
		return Instance{}, err
	}
...
func (s *Service) Update(ctx context.Context, currentID string, in UpdateInput) (Instance, error) {
	...
	defer tx.Rollback()
	result, err := tx.ExecContext(ctx, `UPDATE instances SET id=?,model_id=?,name=?,enabled=?,autoload_enabled=?,always_on=?,priority=?,eviction_enabled=?,idle_unload_seconds=?,gpu_mode=?,gpu_devices=?,tensor_split=?,updated_at=unixepoch() WHERE id=?`,
		i.ID, i.ModelID, i.Name, boolInt(i.Enabled), boolInt(i.Autoload), boolInt(i.AlwaysOn), i.Priority, boolInt(i.EvictionEnabled), i.IdleUnloadSeconds, i.GPUMode, joinDevices(i.GPUDevices), nullString(i.TensorSplit), currentID)
	if err != nil {
		return Instance{}, err
	}
...
func (s *Service) Duplicate(ctx context.Context, id string) (Instance, error) {
	...
	for n := 1; n < 1000; n++ {
		...
		copy, err := s.Create(ctx, CreateInput{
			...
			GPUDevices: append([]string(nil), base.GPUDevices...), TensorSplit: base.TensorSplit, Options: opts,
		})
		if err == nil {
			return copy, nil
		}
...
func (s *Service) Get(ctx context.Context, id string) (Instance, error) {
	return scan(s.db.QueryRowContext(ctx, `SELECT id,model_id,name,enabled,autoload_enabled,always_on,priority,eviction_enabled,idle_unload_seconds,gpu_mode,gpu_devices,tensor_split FROM instances WHERE id=?`, id))
}

func (s *Service) List(ctx context.Context) ([]Instance, error) {
	return s.list(ctx, `SELECT id,model_id,name,enabled,autoload_enabled,always_on,priority,eviction_enabled,idle_unload_seconds,gpu_mode,gpu_devices,tensor_split FROM instances ORDER BY name,id`)
}

func (s *Service) ListByModel(ctx context.Context, modelID string) ([]Instance, error) {
	return s.list(ctx, `SELECT id,model_id,name,enabled,autoload_enabled,always_on,priority,eviction_enabled,idle_unload_seconds,gpu_mode,gpu_devices,tensor_split FROM instances WHERE model_id=? ORDER BY name,id`, modelID)
}
...
func scan(row scanner) (Instance, error) {
	...
	if err := row.Scan(&i.ID, &i.ModelID, &i.Name, &enabled, &autoload, &alwaysOn, &i.Priority, &eviction, &i.IdleUnloadSeconds, &i.GPUMode, &devices, &split); err != nil {
		return Instance{}, err
	}
	i.Enabled = enabled != 0
...
func normalize(in CreateInput) (Instance, error) {
	...
	return Instance{
		...
		TensorSplit: strings.TrimSpace(in.TensorSplit),
	}, nil
}
