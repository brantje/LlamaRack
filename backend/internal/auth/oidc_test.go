...
func TestOIDCProviderLifecycleTestingAndLockoutPolicy(t *testing.T) {
	...
	badSecret := ""
	if _, err := f.manager.CreateProvider(ctx, OIDCProviderInput{Name: "x", Issuer: "not-a-url", ClientID: "id", ClientSecret: &badSecret}); err == nil {
		t.Fatal("invalid issuer should fail")
	}
	validNoSecret := idp.input(nil)
	if _, err := f.manager.CreateProvider(ctx, validNoSecret); err == nil {
		t.Fatal("missing client secret should fail")
	}
	...
	if err != nil || len(listed) != 1 || listed[0].ID != provider.ID {
		t.Fatalf("providers=%+v err=%v", listed, err)
	}
	public, err := f.manager.PublicProviders(ctx)
	if err != nil || len(public) != 1 || public[0].Name != "Primary" {
		t.Fatalf("public=%+v err=%v", public, err)
	}

	tested, err := f.manager.TestProvider(ctx, provider.ID)
	if err != nil || !tested.LastTestSucceeded || tested.LastTestedAt == nil {
		t.Fatalf("tested=%+v err=%v", tested, err)
	}
	if usable, err := f.manager.CanDisableLocalLogin(ctx); err != nil || !usable {
		t.Fatalf("usable=%v err=%v", usable, err)
	}
	if _, err := f.settings.Set(ctx, settings.LocalLoginEnabled, false); err != nil {
		t.Fatal(err)
	}

	unchanged := idp.input(nil)
	kept, err := f.manager.UpdateProvider(ctx, provider.ID, unchanged)
	if err != nil || !kept.LastTestSucceeded {
		t.Fatalf("unchanged update=%+v err=%v", kept, err)
	}
	changed := unchanged
	changed.Name = "Changed"
	if _, err := f.manager.UpdateProvider(ctx, provider.ID, changed); !errors.Is(err, ErrAuthLockoutRisk) {
		t.Fatalf("changed last provider err=%v", err)
	}
	disabled := unchanged
	disabled.Enabled = false
	if _, err := f.manager.UpdateProvider(ctx, provider.ID, disabled); !errors.Is(err, ErrAuthLockoutRisk) {
		t.Fatalf("disable last provider err=%v", err)
	}
	if err := f.manager.DeleteProvider(ctx, provider.ID); !errors.Is(err, ErrAuthLockoutRisk) {
		t.Fatalf("delete last provider err=%v", err)
	}
	...
	if _, err := f.manager.TestProvider(ctx, second.ID); err != nil {
		t.Fatal(err)
	}
	if err := f.manager.DeleteProvider(ctx, provider.ID); err != nil {
		t.Fatal(err)
	}
	if _, err := f.manager.GetProvider(ctx, provider.ID); err == nil {
		t.Fatal("deleted provider should not be readable")
	}

	if _, err := f.settings.Set(ctx, settings.LocalLoginEnabled, true); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || updated.Enabled || updated.LastTestSucceeded || updated.LastTestedAt != nil {
		t.Fatalf("disabled provider=%+v err=%v", updated, err)
	}
	public, err = f.manager.PublicProviders(ctx)
	if err != nil || len(public) != 0 {
		t.Fatalf("disabled public providers=%+v err=%v", public, err)
	}
	if err := f.manager.DeleteProvider(ctx, second.ID); err != nil {
		t.Fatal(err)
	}
}
...
func TestOIDCStartCallbackAndOneTimeExchange(t *testing.T) {
	...
	if _, err := f.manager.Start(ctx, provider.ID, false, "192.0.2.1", "agent", ""); err == nil {
		t.Fatal("OIDC start without external URL should fail")
	}
	...
	idp.mu.Lock()
	idp.nonce = transaction.Nonce
	idp.mu.Unlock()

	if _, err := f.manager.CompleteCallback(ctx, provider.ID, "missing", "code", "https://manager.example.test/base"); err == nil {
	...
	if code == "" {
		t.Fatalf("frontend exchange URL missing code: %s", frontendURL)
	}
	...
	if _, _, err := f.auth.AuthenticateBearer(ctx, result.AccessToken); err != nil {
		t.Fatalf("OIDC bearer token invalid: %v", err)
	}
	if _, err := f.manager.Exchange(code); err == nil {
		t.Fatal("exchange code should be one-time")
	}

	if _, err := f.manager.CompleteCallback(ctx, provider.ID, state, "code", "https://manager.example.test/base"); err == nil {
		t.Fatal("OIDC state should be one-time")
	}
	f.manager.mu.Lock()
	...
	f.manager.mu.Unlock()
	if transactionStillPresent || exchangeStillPresent {
		t.Fatal("expired OIDC transient state was not cleaned up")
	}
}
...
func TestOIDCIdentityResolutionLinkingAndHelpers(t *testing.T) {
	...
	if _, err := f.manager.LinkIdentity(ctx, operator.ID, provider.ID, "https://wrong.example", "subject"); err == nil {
		t.Fatal("mismatched issuer should fail")
	}
	if _, err := f.manager.LinkIdentity(ctx, operator.ID, provider.ID, "", " "); err == nil {
	...
	if identity.Issuer != provider.Issuer || identity.UserID != operator.ID {
	...
	if err != nil || len(items) != 1 || items[0].ID != identity.ID {
		t.Fatalf("user identities=%+v err=%v", items, err)
	}
	resolved, err := f.manager.resolveIdentity(ctx, provider, "operator-subject", "ignored")
	if err != nil || resolved.ID != operator.ID {
		t.Fatalf("existing identity resolved=%+v err=%v", resolved, err)
	}
	if err := f.manager.UnlinkIdentity(ctx, identity.ID); err != nil {
		t.Fatal(err)
	}
	if err := f.manager.UnlinkIdentity(ctx, identity.ID); !errors.Is(err, context.Canceled) && err == nil {
		t.Fatal("unlinking missing identity should fail")
	}

	if _, err := f.settings.Set(ctx, settings.OIDCJITProvisioningEnabled, false); err != nil {
		t.Fatal(err)
	}
	if _, err := f.manager.resolveIdentity(ctx, provider, "jit-off", "new-user"); !errors.Is(err, ErrOIDCLinkRequired) {
		t.Fatalf("JIT disabled err=%v", err)
	}
	if _, err := f.settings.Set(ctx, settings.OIDCJITProvisioningEnabled, true); err != nil {
		t.Fatal(err)
	}
	created, err := f.manager.resolveIdentity(ctx, provider, "jit-new", "new-user")
	if err != nil || created.Username != "new-user" {
		t.Fatalf("JIT user=%+v err=%v", created, err)
	}
	if _, err := f.manager.resolveIdentity(ctx, provider, "too-short", "x"); err == nil {
		t.Fatal("too-short JIT username should fail")
	}

	if _, err := f.manager.resolveIdentity(ctx, provider, "admin-subject", admin.Username); !errors.Is(err, ErrOIDCUsernameTaken) {
		t.Fatalf("username collision err=%v", err)
	}
	if _, err := f.settings.Set(ctx, settings.OIDCAutoLinkEnabled, true); err != nil {
		t.Fatal(err)
	}
	autoLinked, err := f.manager.resolveIdentity(ctx, provider, "admin-subject", admin.Username)
	if err != nil || autoLinked.ID != admin.ID {
		t.Fatalf("auto-linked user=%+v err=%v", autoLinked, err)
	}

	if got := normalizeScopes([]string{"email, profile", "email", "openid"}); strings.Join(got, ",") != "openid,email,profile" {
		t.Fatalf("normalizeScopes=%v", got)
	}
	if !stringSlicesEqual([]string{"email", "openid"}, []string{"openid", "email"}) || stringSlicesEqual([]string{"openid"}, []string{"openid", "email"}) {
		t.Fatal("stringSlicesEqual mismatch")
	}
	if got := oidcUsername(provider, map[string]any{"email": " person@example.test "}, "sub"); got != "person@example.test" {
		t.Fatalf("oidcUsername email=%q", got)
	}
	if got := oidcUsername(provider, map[string]any{}, "sub"); got != "oidc-sub" {
		t.Fatalf("oidcUsername fallback=%q", got)
	}
	if _, err := oidcCallbackURL("", provider.ID); err == nil {
		t.Fatal("empty callback base should fail")
	}
	callback, err := oidcCallbackURL("https://manager.example.test/base/?old=1#fragment", provider.ID)
	if err != nil || !strings.Contains(callback, "/base/api/v1/auth/oidc/") || strings.Contains(callback, "old=1") || strings.Contains(callback, "fragment") {
		t.Fatalf("callback=%q err=%v", callback, err)
	}
	if _, err := frontendExchangeURL("not-a-url", "code"); err == nil {
		t.Fatal("invalid frontend base should fail")
	}
	frontend, err := frontendExchangeURL("https://manager.example.test/base?keep=1#fragment", "code")
	if err != nil || !strings.Contains(frontend, "oidc_exchange=code") || !strings.Contains(frontend, "keep=1") || strings.Contains(frontend, "fragment") {
...
func TestOIDCDiscoveryAndProviderErrorPaths(t *testing.T) {
	...
	if err := f.manager.probeJWKS(ctx, badDiscovery.URL+"/jwks"); err == nil {
		t.Fatal("empty JWKS should fail")
	}
	if _, err := f.manager.GetProvider(ctx, "missing"); err == nil {
		t.Fatal("missing provider should fail")
	}
	if err := f.manager.DeleteProvider(ctx, "missing"); err == nil {
		t.Fatal("deleting missing provider should fail")
	}
}
