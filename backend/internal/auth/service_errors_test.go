...
func TestDisabledAndExpiredCredentials(t *testing.T) {
	...
	if _, _, err := s.Login(ctx, "admin", "correct-horse-battery"); err == nil { t.Fatal("disabled user should not login") }
...
func TestDatabaseErrors(t *testing.T) {
	...
	if _, _, err := s.Login(ctx, "admin", "correct-horse-battery"); err == nil { t.Fatal("expected login failure") }
	...
	if _, _, err := s.CreateAPIKey(ctx, "key"); err == nil { t.Fatal("expected create key DB error") }
