...
- One inference API key named `LiteLLM`. It is listed on `GET /api/v1/api-keys`; there is no `hidden` column on `api_keys`. Operators may set its `instance_ids` allowlist (empty means all instances).
...
Each published Instance becomes a LiteLLM model row:

```json
{
  "model_name": "<instance.id>",
  "litellm_params": {
    "model": "openai/<instance.id>",
  ...
  "model_info": {
    ...
    "llamarack_instance_id": "<instance.id>"
  }
}
```
...
2. Create missing enabled Instances (`POST /model/new`).
3. Update drifted owned rows (`POST /model/update`), including rename-in-place when an Instance ID changes.
4. Delete owned rows whose Instance is disabled or gone (`POST /model/delete` by LiteLLM `model_info.id`).
5. Never mutate unmanaged models.
...
- Enabled Instances appear in LiteLLM with the owned shape above.
...
- Public PATCH of the managed key cannot change name or owner. Its `instance_ids` allowlist is editable. Public rotate of the managed key returns 404.
