...
`/metrics` exposes Prometheus-format manager metrics and can be protected with its own configured authentication token. That token is stored encrypted and is never returned by the settings API.
...
Secrets such as Hugging Face tokens, OIDC client secrets, and the Prometheus `/metrics` bearer token remain server-side and are not returned as plaintext after storage.
