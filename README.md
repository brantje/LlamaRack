...
For production, the default Compose example runs the published single-container image from GHCR:
...
Open http://localhost:8888. The Nuxt frontend and Go API are served from the same container and origin.

For NVIDIA hosts, use the CUDA image override:
...
The original source-build Compose setup is preserved as the development example:
...
```bash
docker compose -f docker-compose.dev.yml -f docker-compose.dev.nvidia.yml up -d --build
```
...
The NVIDIA Compose overrides explicitly reserve NVIDIA GPUs and request `NVIDIA_VISIBLE_DEVICES=all` plus `NVIDIA_DRIVER_CAPABILITIES=compute,utility`. Production defaults to `ghcr.io/brantje/llamacpp-manager:latest-cuda`; development defaults the backend build to `ghcr.io/ggml-org/llama.cpp:server-cuda`. This is important because GPU telemetry and scheduling can see `CUDA0` through `nvidia-smi`, but a CPU-only `llama-server` will reject `--device CUDA0` as an invalid device.

After changing NVIDIA runtime/image settings in development, recreate the backend container rather than merely restarting it:

```bash
docker compose -f docker-compose.dev.yml -f docker-compose.dev.nvidia.yml up -d --build --force-recreate backend
```

Verify both the NVIDIA runtime and the llama.cpp backend from inside the same development backend container:

```bash
docker compose -f docker-compose.dev.yml -f docker-compose.dev.nvidia.yml exec backend nvidia-smi
docker compose -f docker-compose.dev.yml -f docker-compose.dev.nvidia.yml exec backend nvidia-smi pmon -c 1
docker compose -f docker-compose.dev.yml -f docker-compose.dev.nvidia.yml exec backend /app/llama-server --list-devices
```

`--list-devices` should include the CUDA device(s) the manager will pass to `--device`. If `nvidia-smi` works but `llama-server --list-devices` does not show CUDA, the container is using a non-CUDA llama.cpp image. If a custom `LLAMA_IMAGE` is supplied while using the NVIDIA development override, it must itself be CUDA-enabled.
...
The default development backend image uses the standard llama.cpp server image. To test a different llama.cpp image variant outside the NVIDIA override, set `LLAMA_IMAGE` while building, for example:

```bash
LLAMA_IMAGE=ghcr.io/ggml-org/llama.cpp:server-cuda docker compose -f docker-compose.dev.yml up --build
```
