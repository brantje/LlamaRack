...
At startup LlamaRack creates or repairs the configuration directory to mode `0700` and the SQLite database plus `-wal`/`-shm` sidecars to mode `0600`. It does not rely on process umask. If the underlying filesystem cannot honor Unix permission bits (for example some FAT/NTFS/CIFS volume mounts), startup fails with an error asking you to place `LLAMARACK_DATABASE_PATH` on a Unix filesystem that supports `chmod`, or to fix the volume mount. The models volume is left at `0755`.
